...
/**
 * 网络判断工具
 * 来源: https://github.com/Blankj/AndroidUtilCode/blob/master/lib/utilcode/src/main/java/com/blankj/utilcode/util/NetworkUtils.java
 */
public final class NetworkUtils {
    ...
    public static void openMobileData(Context context, boolean enable) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            if (ShellUtil.checkRootPermission()) {// 5.0 以上需要root
                String cmd = enable ? "svc data enable" : "svc data disable";
                ShellUtil.execCommand(new String[]{cmd, "exit"}, true);
            }
        } else { // 5.0以下可用
            try {
    ...
    public static boolean isAvailableByPing(String ip) {
        ...
        if (result.errorMsg != null) {
            LoggerUtil.d("NetworkUtils", "isAvailableByPing() called" + result.errorMsg);
        }
        if (result.successMsg != null) {
            LoggerUtil.d("NetworkUtils", "isAvailableByPing() called" + result.successMsg);
        }
        return ret;
    }
